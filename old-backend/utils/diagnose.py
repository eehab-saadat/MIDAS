...
def generate_diagnosis(data, image):
    ...
    try:
        ...
        response = requests.post(
            OLLAMA_URL, json=payload, timeout=800
        )  # 5 minutes timeout
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if "diagnosis" not in parsed_json or "reasoning" not in parsed_json:
                    ...
                    return {
                        "error": "Invalid response format: missing 'diagnosis' or 'reasoning' field"
                    }

                logger.info(
                    f"  ✅ Successfully parsed diagnosis: {parsed_json['diagnosis'][:50]}..."
                )
                return parsed_json
        ...
        return {
            "error": "Unable to connect to Ollama. Make sure Ollama is running locally on port 11434."
        }
    except requests.exceptions.Timeout:
        logger.error("  ❌ Request to Ollama timed out")
        return {
            "error": "Request timed out. The model may be processing. Please try again."
        }
    except Exception as e:
